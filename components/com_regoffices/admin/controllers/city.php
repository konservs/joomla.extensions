 ...
 * @author Andrii Biriev, a@konservs.com
 */
class RegofficesControllerCity extends JControllerForm{
	...
	public function save($key = null, $urlVar = null){
		...
		if(!$model->save($data)){
			...
			//$app->enqueueMessage('Could not save data form! form data='.var_export($data,true), 'error');
			$app->enqueueMessage('City succesfully saved.', 'message');
			}
	...
	public function delete(){			
		...
		$this->setRedirect('index.php?option='.$this->option.'&view='.$this->view_list);
		$app->enqueueMessage(JText::_('COM_REGOFFICES_CITY_DELETED').' id: '.var_export(implode(', ', $cid),true), 'message');	
		return true;	
		}		
	}
